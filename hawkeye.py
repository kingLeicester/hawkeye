...
class SignalDenoisor:
	...
	def meidan_filter(data_frame, median_with_max , sampling_rate):
		...
		filter_width = sampling_rate * median_with_max
	...
	def remove_blinks(data_frame, median_with_max, max_blink_second, sampling_rate):
		...
		max_blink_samples = int(np.round(max_blink_second * sampling_rate))
	...
	def detect_saccade(data_frame, sampling_rate):
		gaze_array = data_frame[['x_deblinked', 'y_deblinked']].fillna(0).values
		saccade_detector = nystrom_saccade_detector.AdaptiveDetector(gaze_array, sampling_rate, threshold_sd_scale=3)
