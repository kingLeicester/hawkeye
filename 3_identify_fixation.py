...
for image in postDenoise_imageList:
	...
	if number_interpolated + number_original + number_missing == total_number_samples:
		...
		valid_data_ratio = round(((number_original + number_interpolated)/total_number_samples) * 100, 2)
		...
		print (f"percent valid data :{valid_data_ratio}% ({number_original + number_interpolated}/{total_number_samples})")
	...
	if aoi.empty:
		...
		true_fixation_labeld_df = fixation_detector.identify_missing_data(true_fixation_labeld_df)
		true_fixation_labeld_df = fixation_detector.fill_missing_image_value(image, true_fixation_labeld_df)
		...
		true_fixation_in_IAPS_list = fixation_detector.detect_fixation(true_fixation_df)
	...
	if single_rectangle_aoi_df.empty:
		...
		if number_aoi >= 1:
			while aoi_counter < number_aoi:
				...
				merged = true_fixation_df.merge(single_rectangle_aoi_df.iloc[[aoi_counter]], on=['image'], right_index=True)
	...
	if single_ellipse_aoi_df.empty:
		...
		if number_aoi >= 1:
			while aoi_counter < number_aoi:
				...
				merged = true_fixation_labeld_df.merge(single_ellipse_aoi_df.iloc[[aoi_counter]], on=['image'], right_index=True)#.set_index(fixation_df.index)
				...
				ellipse_computed_df = gaze_compiler.ellipse_compute_gaze_in_AOI(cleaned_fixation_df)

				true_fixation_df = ellipse_computed_df.loc[ellipse_computed_df['final_data_type'] == "true_fixation"]

				#fixation_in_aoi_df.to_csv("/study/midusref/DATA/Eyetracking/david_analysis/data_processed/sample.csv")
				...
				#fixation_in_aoi_df = fixation_in_aoi_df['ellipse_value'] <= 1
				fixation_in_aoi_df = true_fixation_df.loc[true_fixation_df['ellipse_value'] <= 1]
				fixation_out_aoi_df = true_fixation_df.loc[true_fixation_df['ellipse_value'] > 1]
				# Same thing as dropping gaze points out of the grid
				#fixation_in_aoi_df.drop(fixation_in_aoi_df.index[fixation_in_aoi_df['ellipse_value'] > 1 ], inplace=True)
				#fixation_in_aoi_df.drop(fixation_in_aoi_df.index[fixation_in_aoi_df['ellipse_value'].isnull()], inplace=True)
				...
				# if there are at least one fixation in AOIs
					...
					if len(true_fixation_in_AOI_list) > 0:
						...
						# # Verify Total duration of fixation outside of AOI
						# fixation_out_AOI_list = fixation_detector.detect_fixation(fixation_out_aoi_df)
						# total_number_fixations_out_AOI, true_fixation_out_AOI_list = fixation_detector.detect_true_fixation(fixation_out_AOI_list)
						# total_fixation_duration_out_AOI = fixation_detector.compute_total_duration_fixation(true_fixation_out_AOI_list)


						# if float(total_fixation_duration_out_AOI) == float(total_fixation_duration_outside_AOI):
						# 	print ("Fixation duration outside of AOI checks out")
						# 	print ("-----c2. total duration of fixations outside AOI: {}ms({}%)-----".format(str(total_fixation_duration_outside_AOI), str(percent_fixation_out_AOI)))
						# 	print ("")
						# 	total_out_AOI_list.append(str(total_fixation_duration_out_AOI))

						# else: 
						# 	print ("Fixation duration outside of AOI dose NOT check out, using more precise measure")
						# 	print ("-----c2. total duration of fixations outside AOI: {}ms-----".format(str(total_fixation_duration_out_AOI)))
						# 	print ("")
						# 	total_out_AOI_list.append(str(total_fixation_duration_out_AOI))
	...
	###### This part saves per IAPS validity in CSV
	# print ("compiling")
